...
async function submitBtnClict() {
  signName = document.querySelector("#signName").value
  signPass = document.querySelector("#signPass").value
  const result = await fetch('http://localhost:3000/tasks/users', {
    method: 'POST',
...
async function addTaskBtnClicked() {
  //get the D title from the input
  let newTitle = document.querySelector("#new-task");
  let newTaskValue = newTitle.value;
  let userNamet = document.querySelector("#user-Name-input");
  ...
  let newTaskObject = {
    id: generateRandomInteger(0, 10000),
    title: newTaskValue,
    done: false,
    deleted: false,
    usersName: userName
...
async function taskCompleted(btnCompletedElement) {
  let id = btnCompletedElement.parentNode.parentNode.getAttribute('data-task-id');
...
function buildTaskElement(task, index) {
  ...
  taskElement.innerHTML = `<p>${task.title} <span id='userName'>added by  ${task.usersName}<span/></p>
    <div>
   ${task.done ? "" :
